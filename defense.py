...
def forward_embedding_no_grad(hidden_states,input_shape, model=None, tokenizer=None, text_encoder=None):
    with torch.no_grad():
        ...
        # output = [1, 77, 768]
        # print(encoder_outputs.input_shape())
...
def craft_candidate_embed(char_list, tokenizer=None, text_encoder=None):
    prompt_char=''
    # reason length = 75 is that tokenizer will and additional start and end token to each token sentences.
    position_length=75
    candidate_embedding = []
    for i in char_list:
        prompt_char=''
        # [1, 77, 768]  generate a prompt (len = 75 * 2) ". . . . ......" and get embedding from it.
    ...
    # integrate all elements from list into a new dimension vector
    # 63 * [1, 77, 768]  =>  [1, 63, 77, 768]
    candidate_embedding = torch.stack(candidate_embedding,dim=1)
    # [1, 63, 77, 768]  =>  [1, 77, 63, 768]  exchange dimension
...
def train(init_per_sample, sentence, len_prompt, char_list, model, iter_num = 100, eta_u=1, mask=None, tokenizer=None, text_encoder=None):
    ...
    # [1, 77, 63]   one sample, generate a 77 length vector to each 63 chars filled by 1/63.
    ...
    # [1, 77]  start from 6+1 to 6+5+1 replace zero by 1
    fixed_z[0][num_word+1:num_word+len_prompt+1]=1
    # get gradient
    u_tensor.requires_grad = True
    # [1, 77, 63, 768]   Each candidate is a duplicate string of length 75
    ...
    print("DEFENSE", "Batch:", batch_size, "Iter:", iter_num)
    n = 5  # top n for backward
    pgd = PGDattack()
    for i in range(iter_num):
        topn_vector=[] #{loss, vector}
        for j in range(batch_size):
            ## We store top 5 loss in each batch size.
            # [1,77,63]
            ...
            # [1, 77, 63, 768]  63 char lists
            subword_embeddings=candidate_embed.view(1, seq_len, neighbor_num, -1)

            # [1, 77]   [ 6: 6+5]  prompt position len = 77   
            # after view(), discrete_z = [1, 77, 1]
            discrete_z = fixed_z.view(init_per_sample, seq_len, 1)
            discrete_z = discrete_z.cuda()
            # [1, 77, 768] orginal embeddings
            orig_embeddings = orig_embeddings.view(1, seq_len, -1)
            # Keeping other position original, get new embeddings
            # [1, 77, 1] * [1, 77, 768]+ [1,77,63, 1] * [1, 77, 63, 768]   
            # after torch.sum( dim = 2 )  it will be [1, 77, 768]
        ...
        # print(temp_loss.item(), "   ", min(single_loss_list))
        total_loss=0
        loss = 0
        for k in range(len(topn_vector)):
            min_vector = topn_vector[k][1]
            new_embeddings = (1 - discrete_z) * orig_embeddings + discrete_z * torch.sum(min_vector * subword_embeddings, dim = 2)
            new_embeddings=new_embeddings.float()
